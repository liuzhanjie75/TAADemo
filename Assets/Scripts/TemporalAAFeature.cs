...
public class TemporalAAFeature : ScriptableRendererFeature
{
    ...
    private class TemporalAAPass : ScriptableRenderPass
    {
        private static RenderTexture _temp;
        private static RenderTexture _temp1;
        private TemporalAAFeature _feature;
        ...
        private UnityEngine.Camera _prevRenderCamera;
        ...
        private void UpdateValuesFeature()
        {
            TemporalFade = _feature.TemporalFade;
            MovementBlending = _feature.MovementBlending;
            HaltonLength = _feature.HaltonLength;
            JitterSpread = _feature.JitterSpread;
        }
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            UpdateValuesFeature();
            var commandBuffer = CommandBufferPool.Get("TemporalAAPass");
            
            TaaMaterial.SetTexture(TemporalAATexture, _temp);
            var matrix4X4 = renderingData.cameraData.camera.nonJitteredProjectionMatrix.inverse;
            TaaMaterial.SetMatrix(InvP, matrix4X4);
            matrix4X4 = _prevViewProjectionMatrix * renderingData.cameraData.camera.cameraToWorldMatrix;
            TaaMaterial.SetMatrix(FrameMatrix, matrix4X4);
            TaaMaterial.SetFloat(Fade, TemporalFade);
            TaaMaterial.SetFloat(Blending, MovementBlending);
            TaaMaterial.SetInt(FlipUV, UseFlipUV ? 1 : 0);
            
            Blit(commandBuffer, BuiltinRenderTextureType.CurrentActive, _temp1, TaaMaterial);
            Blit(commandBuffer, _temp1, renderingData.cameraData.renderer.cameraColorTargetHandle);

            (_temp, _temp1) = (_temp1, _temp);
            
            context.ExecuteCommandBuffer(commandBuffer);
            CommandBufferPool.Release(commandBuffer);

            _prevViewProjectionMatrix = renderingData.cameraData.camera.nonJitteredProjectionMatrix *
        ...
        public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            _prevRenderCamera = renderingData.cameraData.camera;
                
            ConfigureInput(ScriptableRenderPassInput.Color);
            ConfigureInput(ScriptableRenderPassInput.Depth);

            var cameraTargetDescriptor = renderingData.cameraData.cameraTargetDescriptor;
            if (_temp && cameraTargetDescriptor.width != _temp.width || cameraTargetDescriptor.height != _temp.height)
            {
                Debug.Log("Deleting Render Target: " + cameraTargetDescriptor.width + " " + cameraTargetDescriptor.height);
            ...
            if (!_temp)
            {
                _temp = new RenderTexture(cameraTargetDescriptor.width, cameraTargetDescriptor.height, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat, 0);
                _temp1 = new RenderTexture(cameraTargetDescriptor.width, cameraTargetDescriptor.height, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat, 0);
        ...
        public override void FrameCleanup(CommandBuffer cmd)
        {
            ...
            var renderCamera = _prevRenderCamera;
            if (!renderCamera)
                return;
            ...
            var projectionMatrix = renderCamera.projectionMatrix;
            float2 jitter = (float2)(2 * Halton2364Seq[Time.frameCount % HaltonLength] - 1) * JitterSpread;
            projectionMatrix.m02 = jitter.x / Screen.width;
            projectionMatrix.m12 = jitter.x / Screen.height;
            renderCamera.projectionMatrix = projectionMatrix;
        }
    }

    private TemporalAAPass _temporalAAPass;
    private static readonly int TemporalAATexture = Shader.PropertyToID("_TemporalAATexture");
    private static readonly int InvP = Shader.PropertyToID("_invP");
    private static readonly int FrameMatrix = Shader.PropertyToID("_FrameMatrix");
    private static readonly int Fade = Shader.PropertyToID("_TemporalFade");
    private static readonly int Blending = Shader.PropertyToID("_MovementBlending");
    private static readonly int FlipUV = Shader.PropertyToID("_UseFlipUV");

    public override void Create()
    {
        _temporalAAPass = new TemporalAAPass(this)
        {
            TemporalFade = TemporalFade,
            MovementBlending = MovementBlending,
            TaaMaterial = TaaMaterial,
